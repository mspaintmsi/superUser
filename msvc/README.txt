...
The solution is to link with the unversioned msvcrt.dll that has shipped with all versions of Windows.
...
To do so, the program must be linked statically with the MSVCRT.LIB library. This is not provided with Visual Studio. It must be extracted from the Windows Driver Kit (WDK).
